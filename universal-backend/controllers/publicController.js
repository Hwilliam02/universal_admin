...
import tenantSchemaWillcall from '../schemas/sqlSchemaWillcall.js';
import tenantSchemaJobBoard from '../schemas/sqlSchemaJobBoard.js';
import { sendTrialSignupEmail  } from '../templates/trialSignupEmail.js';
import { generateUUIDv7  } from '../utils/getTanent.js';
...
const completeTrial = async (req, res) => {
  ...
  try {
    ...
    const tenantConnectionSchema = await mysql.createConnection({
    ...
    if (finalDeliveryType === "willcall") {
      await tenantConnectionSchema.query(tenantSchemaWillcall);
    } else if (finalDeliveryType === "job") {
      await tenantConnectionSchema.query(tenantSchemaJobBoard);
    } else {
      await tenantConnectionSchema.query(tenantSchemaWillcall); // Default to willcall
    }

    await tenantConnectionSchema.end();
